...
(() => {
    ...
    anchors.forEach((a, ai) => {
      ...
      for (let i = start; i < next; i++) {
        ...
        if (/^（.+）$/.test(t)) {
          ...
          if (name && (sire || dam)) horses.push({ name, number, sex, sire, dam, nameEl: a, badgeHost: a.parentElement });
          break;
        }
      }
    });
  ...
  shown.forEach(g => {
    g.members.forEach(h => {
      ...
      b.style.cssText = (h.badgeHost ? 'display:block;margin:2px 0 0;' : 'display:inline-block;margin-left:6px;') + 'padding:1px 6px;' +
        'border-radius:9px;font-size:11px;font-weight:bold;color:#fff;' +
        'vertical-align:middle;background:' + g.color + ';';
      if (h.badgeHost) {
        h.badgeHost.style.whiteSpace = 'normal';
        h.badgeHost.style.overflow = 'visible';
        h.badgeHost.appendChild(b);
      } else {
  ...
  const groupHtml = g => '<div style="margin:8px 0;border-left:4px solid ' + g.color + ';padding:5px 8px;">' +
    ...
    g.members.map(m => '<div style="margin-top:3px;">' +
      '<b style="display:inline-block;width:2ch;text-align:right;font-size:16px;">' + (m.number || '') + '</b> ' + m.name +
  ...
  let html = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
    '<strong>兄弟馬：' + shown.length + '組</strong>' +
